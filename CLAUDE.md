...
### 1. URL照合ルール
- **マッチング方式**: ドメイン一致
- 実装時は登録URLと現在のURLのドメイン（プロトコル + ホスト名）を比較
- 例: `https://example.com` を登録すると、`https://example.com/any/path` でマッチ
- 同一ドメインへの複数登録を許可
...
```javascript
{
  reminders: [
    {
      id: "uuid_1",                          // ユニークID
      url: "https://example.com",            // ドメイン一致用URL（通常はドメインのみ）
...
3. リマインダー内容を入力
4. 「登録」ボタンをクリック
5. `chrome.storage.local` に保存
6. 成功メッセージ表示

### 2. 表示・操作フロー
1. ユーザーがWebページにアクセス
2. `content.js` がページ読み込み完了を検知
3. 現在のURLのドメインと登録済みURLのドメインを照合
4. 該当リマインダーを全て取得
...
1. **URL照合テスト**
   - ドメイン一致が正しく機能するか
   - 異なるパスでも同じドメインならマッチするか
   - サブドメインは別ドメインとして扱われるか
   - 複数リマインダーが正しく表示されるか
